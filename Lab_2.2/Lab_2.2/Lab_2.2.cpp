...
void useOfUnion()
{
	...
	Firma Company[100];
	int k = 0;            //номер пункту меню
	while (k != 3)
	{
		int N = 0;
		system("cls");
		cout << "1.Firm with name and quantity of products" << endl;
		cout << "2.Firm with adress and termin" << endl;
		cout << "3.Exit" << endl;
		cout << "Enter your choose: ";
		cin >> k;                  //вибір потрібного пункту меню
		switch (k)
		{
		case 1: {                   
			int Ans = 0;                 //ознака завершення введення даних
			do{
				cout << "Please enter\nName of firm:" << endl;
				cin >> Company[N].First.Name;
				cout << "How many products are produced by " << Company[N].First.Name << "?" << endl;
				cin >> Company[N].First.Quantity;
				N++;
				cout << "Do you want to continue entering?(if yes enter '1')" << endl;
				cin >> Ans;
			} while (Ans == 1);
			cout << "Output" << endl;
			cout << setw(10) << "Name" << setw(15) << "Quantity" << endl;
			for (int i = 0; i < N; i++)
			{
				cout << i + 1 << setw(9) << Company[i].First.Name << setw(15) << Company[i].First.Quantity << endl;
			}
			system("pause");
			break; 
		}
		case 2: {
					...
					do{
						cout << "Please enter\nAdress of firm:" << endl;
						cin >> Company[N].Second.Adress;
						cout << "How many days client must wait:" << endl;
						cin >> Company[N].Second.Termin;
						N++;
						cout << "Do you want to continue entering?(if yes enter '1')" << endl;
						cin >> Ans;
					} while (Ans == 1);
					cout << "Output" << endl;
					cout << setw(10) << "Adress" << setw(15) << "Termin" << endl;
					for (int i = 0; i < N; i++)
					{
						cout << i + 1 << setw(9) << Company[i].Second.Adress << setw(15) << Company[i].Second.Termin << endl;
					}
					system("pause");
					break;
		}
...
int main()
{
	while (true)
	{
		...
		cout << "laboratory work number 2 made by Anton Doroshenko, IS-52" << endl;
		cout << "Menu" << endl;
		...
		case 8: Delate();         //видалення даних
			break;
		case 9: useOfUnion();         //використання об'єднань
			break;
		case 10: exit(0);             //вихід з програми
			break;
		default:{ cout << "Please, try again..." << endl;
			system("pause");
			break; }
